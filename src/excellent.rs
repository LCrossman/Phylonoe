...
#[pyfunction]
fn metrics(py: Python, result: &PyTuple) -> PyResult<()> {
    let res = String::new();
...
pub fn preprocess() -> PyResult<(Vec<String>, Vec<Vec<String>>)> {
    // Open files
    let from_1 = BufReader::new(File::open("test/proc1_20.fofn").expect("Failed to open file"));
    let from_2 = BufReader::new(File::open("test/proc2_20.fofn").expect("Failed to open file"));
    let from_3 = BufReader::new(File::open("test/proc3_20.fofn").expect("Failed to open file"));
    let from_4 = BufReader::new(File::open("test/proc4_20.fofn").expect("Failed to open file"));
...
def process_data_py(result, from_auth, word_data):
       ...
       print("shape of features_test_transformed", features_test_transformed.shape)
       # Define colormap
       cmap = plt.cm.RdYlBu
       from sklearn.decomposition import PCA, TruncatedSVD
       pca = PCA(n_components=2, svd_solver="arpack")
       features_test_transformed_pca = pca.fit_transform(features_test_transformed)
       print("now shape is ", features_test_transformed_pca.shape)
       # We only take the two corresponding features
       plt.figure(figsize=(12,9))
       x_min, x_max = features_test_transformed_pca[:, 0].min() - 1, features_test_transformed_pca[:, 0].max() + 1
       y_min, y_max = features_test_transformed_pca[:, 1].min() - 1, features_test_transformed_pca[:, 1].max() + 1
       xx, yy = np.meshgrid(
                   np.arange(x_min, x_max, plot_step), np.arange(y_min, y_max, plot_step)
                   )
       for class_idx in range(num_plots):
               plt.subplot(2, 2, class_idx + 1)
               title = f"Class {class_idx}"
               plt.title(title, fontsize=9)
               Z = predicted == class_idx
               Z = Z.reshape(xx.shape)
               cs = plt.contour(xx, yy, Z, cmap=cmap)

               plt.scatter(
                    features_test_transformed_pca[:, 0],
                    features_test_transformed_pca[:, 1],
                    c = predicted,
       ...
       plt.suptitle("Decision boundaries for each class", fontsize=12)
       plt.tight_layout(h_pad=0.2, w_pad=0.2, pad=2.5)
       plt.show()
       plt.savefig("figuresaved.png")
       return result"#, "process_data_py.py", "process_data_py")?;
   let result: &PyTuple = processing_data.getattr("process_data_py")?.call1((result, from_auth, word_data))?.extract()?;
   println!("result is now {:?}", &result);
   let metric = metrics(py, result);
   Ok(result)
}
